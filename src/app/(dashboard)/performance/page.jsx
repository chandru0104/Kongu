...
// =======================================================
const StudentListItem = ({ student, onClick }) => { // 🆕 Added onClick prop
    const theme = useTheme();
                ...
                <Box display="flex" alignItems="center" flexGrow={1}>
                    ...
                    <Box>
                        ...
                        <Box display="flex" alignItems="center" flexWrap="wrap" sx={{ mt: 0.5 }}>
                            ...
                            {/* Grade/Role */}
                            <Typography variant="body2" fontWeight="medium" sx={{ color: theme.palette.text.primary, mr: 3, display: 'flex', alignItems: 'center' }}>
...
const StudentPerformanceDialog = ({ student, open, onClose }) => {
    ...
    const bestSubject = subjectEntries.reduce((best, [name, score]) => (score > best.score ? { name, score } : best), { name: '', score: -1 });
    const worstSubject = subjectEntries.reduce((worst, [name, score]) => (score < worst.score ? { name, score } : worst), { name: '', score: 101 });
                    ...
                    <Grid item xs={12}>
                        ...
                        >
                            ...
                            <Typography variant='body1'>
                                The student's lowest score is in {worstSubject.name} with {worstSubject.score}. 
                                Focused study and additional practice in this area are recommended to improve the overall percentage.
                            </Typography>
                        </Box>
                    </Grid>
                </Grid>
        ...
        <Box sx={{ backgroundColor: '#f4f6f8', minHeight: '100vh' }}>
            ...
            <Container maxWidth='xl' sx={{ py: 2 }}>
                ...
                <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', mt: 3, p: 1 }}>
                    <Typography variant='body2' color='text.secondary'>
                        Showing {Math.min(startIndex + 1, totalFilteredMembers)} - {Math.min(startIndex + ITEMS_PER_PAGE, totalFilteredMembers)} of {totalFilteredMembers} student members
                        {searchTerm && <Chip label={`Filtered from ${totalMembers}`} size='small' sx={{ ml: 1, color: 'text.secondary' }} />}
                    </Typography>
                    <Pagination 
                        count={totalPages} 
                        page={currentPage} 
